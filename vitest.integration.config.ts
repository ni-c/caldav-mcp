 ...
 *
 * No coverage is collected here. What this suite proves is that a real CalDAV server
 * behaves as the code assumes, which is not a question about which lines ran.
 */
export default defineConfig({
  test: {
    include: ['test/integration/**/*.integration.test.ts'],
    // One story, shared state, one backend. Running the file's blocks against
    // each other in parallel would be a different test every time — and here
    // the second block deletes what the first one created.
