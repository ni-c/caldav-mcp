...
export default defineConfig({
  ...
  themeConfig: {
      ...
      '/guide/': [
        ...
        },
        {
          ...
          items: [
            ...
            // Only for a server that has a guarded tool — see SKILL.md.
    ...
    socialLinks: [
      { icon: 'github', link: 'https://github.com/ni-c/caldav-mcp' },
      // VitePress ships no sponsors icon, so the glyph is Simple Icons'
      // "githubsponsors" (CC0), inlined. Its <title> is stripped — ariaLabel
      // already names the link — and the path carries no `fill`, so
      // `.VPSocialLink > svg { fill: currentColor }` makes it follow the theme.
      {
        icon: {
