...
`list_calendars` long before anybody opens an event. Each entry's name is cleaned like any other text, and its id is the collection's path, printed byte for byte — validated rather than cleaned, because it has to come back as `calendar_id` and match what discovery resolved.
